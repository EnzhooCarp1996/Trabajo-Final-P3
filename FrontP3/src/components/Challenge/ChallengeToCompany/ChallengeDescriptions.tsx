...
import { Button, List, Space, Avatar, Tooltip } from 'antd'
      ...
      <List.Item.Meta
        avatar={
          <Avatar
            ...
            style={{
              backgroundColor:
                challenge.estado === 'activo' ? '#52c41a' : challenge.estado === 'finalizado' ? '#1677ff' : 'red',
            }}
          />
        }
        title={
          ...
          >
            <div style={{ display: 'flex', gap: 16, }}>
              <strong style={{ color: '#69b1ff' }}>{challenge.titulo}</strong>
              <Tooltip title="Ver Propuestas">
                <div
                  onClick={() => navigate(`/ProposalsEditing/${challenge._id}`)}
        ...
        description={
          <Space direction="vertical" size="small" style={{ width: '100%' }}>
            <Space size="large" style={{ fontSize: 13, color: '#8c8c8c' }}>
              <span>{challenge.descripcion}</span>
